...
| `PaymentHistoryList` | 付款紀錄列表 |
| `SubscriptionCard` | 訂閱詳情卡片 |
| `CategoryBreakdown` | 分類支出明細 |
| `CalendarStatsPanel` | 日曆統計面板 |
...
- `usersService.filterUsers()` - 篩選使用者
- `usersService.prepareExportData()` - 準備匯出數據
...
- `rolesService.canDeleteRole()` - 檢查角色是否可刪除
- `rolesService.validateKey()` - 驗證角色識別碼
- `rolesService.validateForm()` - 表單驗證
- `rolesService.togglePermission()` - 切換單一權限
- `rolesService.toggleGroupPermissions()` - 切換群組權限
- `rolesService.filterRoles()` - 篩選角色
...
**服務層功能：**
- `settingsService.filterModulesByPermission()` - 依權限過濾設定模組
- `settingsService.getAllModules()` - 取得所有設定模組
- `settingsService.createDefaultGeneralSettings()` - 建立預設一般設定
- `settingsService.createDefaultNotificationSettings()` - 建立預設通知設定
- `settingsService.formatQuietHoursDisplay()` - 格式化靜音時段顯示
- `settingsService.checkPushPermission()` - 檢查推送通知權限
- `settingsService.requestPushPermission()` - 請求推送通知權限
- `settingsService.countEnabledNotifications()` - 計算已啟用通知數量
- `settingsService.hasChanges()` - 檢查設定是否有變更

**元件列表：**
| 元件 | 用途 |
|------|------|
| `SettingsModuleCard` | 設定模組導航卡片 |
| `ThemeSelector` | 主題選擇器 (亮色/暗色/系統) |
| `NotificationToggleGroup` | 通知開關群組 |
