...
<PageContainer title="設定" description="管理您的帳號設定和偏好">
	<div class="space-y-8" data-testid="settings-modules">
		{#each groupedModules as group}
			<section>
				<div class="mb-4">
					<h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{group.title}</h2>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					{#each group.modules as module}
						<a
							href={module.href}
							class="group block p-5 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-[var(--color-primary-300)] dark:hover:border-[var(--color-primary-700)] hover:shadow-lg transition-all duration-200"
							data-testid="settings-module-{module.id}"
						>
							<div class="flex items-start gap-4">
								<div class={`w-11 h-11 rounded-xl ${module.iconBgColor} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-200`}>
									<svg class={`w-5 h-5 ${module.iconColor}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
										{@html module.icon}
									</svg>
								</div>
								<div class="flex-1 min-w-0">
									<h3 class="text-base font-semibold text-gray-900 dark:text-gray-100 group-hover:text-[var(--color-primary-600)] dark:group-hover:text-[var(--color-primary-400)] transition-colors">
										{module.title}
									</h3>
									<p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400 line-clamp-1">
										{module.description}
									</p>
								</div>
